...
def actualize(topics, request_session=None, last_date=None):
	...
	if actualization_list:
		...
		#postgres: 
		return "INSERT INTO MFNF_EDITS (date, name, topic, number_of_edits) VALUES {} ON CONFLICT (date, name, topic) DO UPDATE number_of_edits = number_of_edits +1".format(', '.join(actualization_list)), str(datetime.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")) #Mediawiki assumes utc-timezone
	else:
